...
export default async function FreelancerPage({ searchParams }: { searchParams: Promise<{ category?: string }> }) {
  ...
  const selectedCategory = [...categories, ...legacyCategories].find((item) => item === category)
  ...
  return <MarketplaceShell name={fullName} role="freelancer" active="dashboard">
    <div className="dashboard-heading"><div><p>FREELANCER PANELİ</p><h1>Günaydın, {fullName.split(' ')[0]}.</h1><span>Sistem, ağ ve güvenlik projelerini keşfet; tekliflerini ve aktif işlerini yönet.</span></div><DashboardDate /></div>
