...
export default async function EmployerPage() {
  ...
  const [{ data: jobs, error: jobsError }, { data: payments }] = await Promise.all([
    supabase.from('jobs').select('*, proposals(id, status)').eq('employer_id', user.id).order('created_at', { ascending: false }).limit(8),
  ...
  return <MarketplaceShell name={fullName} role="employer" active="dashboard">
    ...
    <section className="dashboard-section employer-projects"><div className="dashboard-section-title"><div><p>PROJELERİN</p><h2>İşe alım gündemi</h2></div><Link href="/employer/jobs/new">Yeni ilan →</Link></div>{jobsError && <SetupNotice />}<div className="project-table"><div className="project-table-head"><span>PROJE</span><span>TEKLİFLER</span><span>DURUM</span><span>BÜTÇE</span><span /></div>{jobs?.map((job, index) => <article key={job.id}><div className="project-name"><span className={`project-color ${index % 3 === 1 ? 'blue' : index % 3 === 2 ? 'lime' : ''}`} /><strong>{job.title}</strong></div><Link className="project-proposal-link" href={`/employer/jobs/${job.id}/proposals`}>{job.proposals?.length ?? 0} aday</Link><span className="project-status">{job.status}</span><strong>{formatCurrency(job.budget_max)}</strong><Link className="round-link" href={`/jobs/${job.id}`}>→</Link></article>)}{!jobsError && jobs?.length === 0 && <div className="marketplace-empty"><p>Henüz projen yok. İlk ilanını yayınlayarak başla.</p></div>}</div></section>
