...
export default async function NewJobPage({ searchParams }: { searchParams: Promise<{ error?: string; message?: string }> }) {
  ...
  return <MarketplaceShell name={fullName} role="employer" active="new-job">
    ...
    <form action={createJob} className="marketplace-form job-create-form">
      ...
      <label>İlan başlığı<input name="title" required minLength={5} maxLength={140} placeholder="Örn. SaaS ürünümüz için onboarding tasarımı" /></label>
      <label>Açıklama<textarea name="description" required minLength={20} rows={8} placeholder="İhtiyacı, teslimatları, hedef kitleyi ve varsa referansları anlat." /></label>
      <div className="form-grid two"><label>Kategori<select name="category" required defaultValue=""><option value="" disabled>Seç</option>{categories.map((category) => <option key={category}>{category}</option>)}</select></label><label>Gerekli beceriler<input name="skills" placeholder="Figma, React, SEO (virgülle ayır)" /></label></div>
