    ...
    {job && <div className="job-detail-page">
      ...
      <div className="job-detail-layout">
        ...
        <aside className="job-action-panel">
          ...
          {isOwner && <div className="owner-job-actions"><strong>{proposals?.length ?? 0} teklif</strong><p>{job.status === 'open' ? 'Teklif almaya açık' : job.status === 'assigned' ? 'Çalışma devam ediyor' : job.status === 'completed' ? 'Çalışma tamamlandı' : 'İlan kapalı'}</p>{job.status === 'open' && <Link className="job-edit-button" href={`/employer/jobs/${id}/edit`}>Projeyi düzenle <span aria-hidden="true">→</span></Link>}{job.status === 'assigned' && <form action={completeJob.bind(null, id)}><PendingSubmitButton pendingLabel="Tamamlanıyor…">İşi tamamlandı olarak işaretle</PendingSubmitButton></form>}{accepted && <Link href={`/messages/${accepted.id}`}>Çalışma alanını aç →</Link>}</div>}
        </aside>
      </div>
