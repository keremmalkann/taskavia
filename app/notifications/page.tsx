...
export default async function NotificationsPage({ searchParams }: { searchParams: Promise<{ error?: string; message?: string }> }) {
  ...
  return <MarketplaceShell name={fullName} role={role} active="notifications">
    <div className="marketplace-page-head"><div><p>BİLDİRİM MERKEZİ</p><h1>Gelişmeleri kaçırma.</h1><span>Mesajlar, teklifler ve ödeme hareketleri burada tek akışta toplanır.</span></div><MarkNotificationsRead unread={feed.unreadCount > 0} className="notification-read-all" /></div>
    <Feedback {...params} />
